...
class FeatureExtractor:
    """
    Class for extracting various features from a waveform.
    ...
    def _extract_raw_keypoints(self):
        ...
        return self._append_to_keys(key_points, 'raw_kp_')
    ...
    def _extract_poly1d_keypoints(self):
        ...
        return self._append_to_keys(key_points, 'poly1d_kp_')
    ...
    def _extract_poly2d_keypoints(self):
        ...
        valid, key_points = feature.extract_key_points(poly_wave, self.sampling_rate, show=self.show)
        poly2d_features = self._append_to_keys(key_points, 'poly2d_kp_')
        if valid:
            feat = feature.extract_key_temporal(poly_wave, self.sampling_rate, key_points, norm_method='z_score')
            # features.update(self._append_to_keys(feat, 'key_temp_'))
    ...
    def extract_spectral_features(preprocessed_signal, raw_signal, sampling_rate):
        ...
        return FeatureExtractor._append_to_keys(feat, 'spect_')

    @staticmethod
    def _append_to_keys(input_dict, appendix):
        output_dict = {}
